...
async function getUserOptions() {
  ...
  const dir_fixed_case = current_directory.replace(
    /[a-zA-Z]+/g,
  ...
  const options = await prompts([
    {
      ...
      name: "name",
      message: "Plugin name?",
      initial: oldConf.name || packageConf.name || dir_fixed_case,
    },
    {
      type: "autocomplete",
      name: "category",
      message: "Category?",
      initial: oldConf.category || "Misc",
    ...
    },
    {
      ...
      name: "description",
      message: "Description?",
      initial: oldConf.description || packageConf.description,
    },
    {
      type: "text",
      name: "author",
      message: "Author?",
      initial: oldConf.author || packageConf.author,
    },
    { type: "toggle", name: "css", message: "use CSS?", initial: true },
    {
      type: "toggle",
      name: "git",
      message: "use GIT tags for versioning?",
      initial: true,
    },
    {
      type: "toggle",
      name: "usechangelog",
      message: "Include changelog?",
      initial: true,
    },
    {
      type: "toggle",
      name: "eslint",
      message: "use ESLint for CodeStyle checks?",
      initial: true,
    },
    {
      type: "text",
...
function commandServe() {
  const proc = spawn(
    "node",
    [MYDIR + "/config/fileserver.js"].concat(process.argv.slice(3)),
    { stdio: "inherit" },
  );
