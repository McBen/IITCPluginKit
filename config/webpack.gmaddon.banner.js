...
import webpack_source from "webpack-sources";
import Compilation from "webpack/lib/Compilation.js";
...
export default class GMAddonBannerPlugin {
  constructor(options) {
    if (arguments.length > 1)
      throw new Error(
        "GMAddonBannerPlugin only takes one argument (pass an options object)",
      );
      ...
      compilation.hooks.processAssets.tap(plugin, () => {
        compilation.chunks.forEach((chunk) => {
          chunk.files.forEach((file) => {
            ...
            return (compilation.assets[file] = new webpack_source.ConcatSource(
              banner,
              "\n",
